...
function compareCharCodeAtAndCodePointAt(string, index) {
  ...
  return `charCodeAt:${string.charCodeAt(
    index
  )}, codePointAt:${string.codePointAt(index)}, charAt:${string.charAt(
    index
  )}, at: ${string.at(index)}`;
}
...
function trimString(original, side) {
  ...
  if (side === "start") {
    return original.trimStart(original, side);
  } else if (side === "end") {
    return original.trimEnd(original, side);
  } else if (side === "both") {
    return original.trim(original, side);
  } else {
